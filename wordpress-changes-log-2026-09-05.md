...
### Tenth wave, 6 September 2026: pension estate deletion (decision by the owner, in progress)

The owner confirmed the pension claims line is in run off and instructed deletion of ALL pension related pages, superseding the earlier consolidation recommendation. Execution: Claude moved 25 respondent profiles to the Bin over the API (recoverable for 30 days); per page permission prompts made the API route impractical for the remainder, so the other 108 profiles, 16 hub/stray/compliance pages and the SIPP blog post (4252) were handed to grokbot in `grokbot-pension-deletion-2026-09-06.md`, which also amends the v2 brief (3098 leaves the noindex list, the local authority duplicate redirect falls away, the B8 pension report is moot). After grokbot's report, Claude will fix internal links pointing at deleted pension URLs (at least one live blog post links to /pension-claims/sipp-claims/) and verify the estate is gone. Record keeping note: the pension pre contract and terms of engagement pages were part of the deletion instruction; the firm's internal document retention (not the website) remains the compliance record for run off cases.
